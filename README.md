...
## 苦労した点
#### 確認画面表示後、入力画面に戻った場合に再表示されるデータ<br>
見積から持ってきたデータ、単価から持ってきたデータ、入力画面で新たに入力したデータ、それぞれのデータがnullの場合など、パターンが複数あったためです。講師に三項演算子を教えてもらい対処しました。
#### 単価登録先が全ユーザー（null）の場合、ログインユーザーが「管理者」か「一般ユーザー」かで表示内容を変える機能。<br>
ifの中にifを入れることで対処しました。
#### 検索部分<br>
画面の占領範囲を小さくするためにタブにしました。各タブで検索後も別のタブ検索内容が保存されるようにして、再検索しやすいようにしました。
<br>
<br>
