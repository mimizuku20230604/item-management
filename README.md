...
- 見積・検索機能
    ...
    - 見積一覧で最初に表示するのは、有効期限内のものにしました。（表示件数が多いと画面表示までに時間がかかるため。）

- 備考欄
    - 顧客・商品に対して、備考（メモ）を残せるようにしました。
...
- 確認のしやすさ（ミス軽減・効率化）
    ...
    - 登録・発注がリアルタイムでわかるように、ユーザーと管理者にメール配信するようにしました。
    - 入力箇所にはminやmaxなどの制限を入れているので、ユーザーはすぐにエラーがわかり、修正できます。
    - 単価登録の際、重複が発生しないように、「顧客名」と「商品名」で同じ組み合わせがある場合はエラーになるようにしました。
    - アクティブセル以外の数量・金額は、カンマ区切りで表示するようにしました。
    - 登録後の確認画面の備考は、自動で高さを調節し、ひと目で全文が見えるようにしました。
    - 登録・修正画面では、顧客・商品名を選択すると、すぐに顧客備考・商品備考を表示するようにしました。
    - 合計金額は、単価・数量を入力後すぐに、計算結果を表示するようにしました。
    - 日本語翻訳。（一括ダウンロードに加え、気になるところは個別修正。）

- その他
    - セッションが切れた場合、自動でログイン画面へ移動するようにしました。（自分で再度URLを叩く作業を省略。）
    - 単価は小数点第2まで、合計金額は四捨五入して整数になるようにしました。
    - 単価登録で顧客をnullにした場合、「全ユーザー」と表示するようにしました。
<br>
<br>
