...
- 見積・検索機能
    - 見積を作成したら、あとは一般ユーザーが自分で発注できるように作成しました。
    - 見積一覧の検索機能には「あいまい検索」と「詳細検索」を実装し、多様な検索ニーズに対応できるようにしました。
    - 見積一覧で最初に表示されるのは有効期限内のものにしました。（表示件数が多いと画面表示までに時間がかかるため。）

- 備考欄
    - 顧客・商品に対して備考（メモ）を残せるようにしました。
    - 各作成画面に顧客備考・商品備考を表示させ、備考（メモ）を見ながら作成できるようにしました。
    - 一般ユーザーに対しては、備考スペースを有効活用するため、新商品情報を表示するようにしました。
 
- 確認のしやすさ（ミス軽減・効率化）
    - 各登録には確認画面を作成し、確認後問題なければ登録できるようにしました。
    - 登録・発注がリアルタイムでわかるように、ユーザーと管理者にメール配信されるようにしました。
    - 入力箇所にはminやmaxなどの制限を入れているので、ユーザーはすぐにエラーがわかり、修正できます。
    - 単価登録の際、重複が発生しないように、「顧客名」と「商品名」で同じ組み合わせがある場合はエラーになるように設定しました。
    - アクティブセル以外の数量・金額は、カンマ区切りで表示するようにしました。
    - 登録後の確認画面の備考は自動で高さが調節され、ひと目で全文が見えるように設定しました。
    - 登録・修正画面では、顧客・商品名を選択すると、すぐに顧客備考・商品備考が表示されるように設定しました。
    - 合計金額は、単価・数量を入力後すぐに計算結果が表示されるようにしました。
    - 日本語翻訳。（一括ダウンロードに加え、気になるところは個別修正。）

- その他
    - セッションが切れた場合、自動でログイン画面へ移動するようにしました。（自分で再度URLを叩く作業を省略。）
